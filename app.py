    ...
    try:
        ...
        for wk in range(1, 33):
            ...
            if wk <= total_weeks:
                ...
                lines.append(f"WK{wk_str}: MON={d1} TUE={d2} WED={d3} THU={d4} FRI={d5} SAT={d6} SUN={d7}")
            else:
                lines.append(f"WK{wk_str}: MON= TUE= WED= THU= FRI= SAT= SUN=")
...
def validate():
    """
    Post-processing validation endpoint.
    Takes Claude's generated plan JSON and unavailable_days.
    Enforces unavailable days and returns corrected JSON.
    """
    try:
        data = request.get_json(force=True, silent=False)
        if data is None:
        ...
        for wk in range(1, 33):
            wk_str = f"{wk:02d}"
            if wk <= total_weeks:
                for day_name, day_key in day_to_key.items():
                    if day_name in unavailable_days:
                        session_key = f"WK{wk_str}_{day_key}"
                        current_value = plan.get(session_key, "REST")
                        if current_value and current_value != "REST" and "RACE DAY" not in str(current_value):
                            plan[session_key] = "REST"
                            corrections.append(f"WK{wk_str}_{day_key}: corrected to REST")
            else:
                for day_key in ["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"]:
                    plan_key = f"WK{wk_str}_{day_key}"
                    if plan.get(plan_key):
                        plan[plan_key] = ""
                        corrections.append(f"WK{wk_str}_{day_key}: cleared phantom week")
